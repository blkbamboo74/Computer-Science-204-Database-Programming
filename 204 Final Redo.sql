...
INSERT INTO borrower(borrowerid, clientid, bookid, borrowdate)
...
-- Confirmed borrower data: 300 rows confirmed
 ...
 -- 1. Display all the contents of the Client table
 SELECT *
 FROM client;
...
ClientLastName, 
YEAR(curdate()) - clientDOB AS ClientDOB, 
Occupation
FROM Client;
...
a.AuthorLastName
FROM Author a
JOIN Book ON a.AuthorID = Book.AuthorID
JOIN Borrower b ON Book.BookID = b.BookID
WHERE YEAR(b.BorrowDate) = 2017
GROUP BY a.AuthorID
...
SELECT c.Occupation, AVG(BookCount) AS AvgBooksBorrowed
FROM Client c
LEFT JOIN (
SELECT ClientID, COUNT(BorrowID) AS BookCount
FROM Borrower
GROUP BY ClientID
) AS BorrowCounts ON c.ClientID = BorrowCounts.ClientID
GROUP BY c.Occupation;

-- 10. Create a VIEW and display the titles that were borrowed by at least 20% of clients

CREATE VIEW PopularBooks AS
SELECT Book.BookID, Book.BookTitle, COUNT(DISTINCT b.ClientID) AS BorrowerCount
FROM BOOK
JOIN Borrower ON Book.BookID = b.BookID
GROUP BY Book.BookID, Book.BookTitle;
-- Display titles borrowed by at least 20% of clients
SELECT BookID, BookTitle, BorrowerCount
FROM PopularBooks
WHERE BorrowerCount >= (SELECT COUNT(DISTINCT ClientID) * 0.2 FROM Client);

-- Currently stuck on this one.  Have to come back to it!
...
SELECT DATE_FORMAT(BorrowDate, '%Y-%m') AS BorrowMonth,
...
SELECT FLOOR(YEAR(CURDATE()) - c.ClientDOB) AS Age,
AVG(BookCount) AS AvgBorrows
FROM Client c
LEFT JOIN ( SELECT ClientID, 
COUNT(BorrowID) AS BookCount
FROM Borrower
GROUP BY ClientID
) AS BorrowCounts ON c.ClientID = BorrowCounts.ClientID
GROUP BY Age;
